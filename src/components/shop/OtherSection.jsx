...
const OtherSection = () => {
  ...
  useEffect(() => {
    ...
    const filteredCards = storedCards.filter(card =>
      card.category.toLowerCase() === "other"
    );
  ...
  const handleNextClick = () => {
    setCurrentIndex((prevIndex) =>
      Math.min(prevIndex + 4, cards.length - 4)
    );
  };
      ...
      <div key={index} className="card-item">
        <div className="card">
          ...
          <div className="card-content">
            <h3>{card.title}</h3>
            <div>
              <p className="priceshop"><strong>Price:</strong> {card.price} Dh</p>
            </div>
  ...
  return (
    <section className="main-section">
      <h2 style={{ textAlign: 'center' }}>Autres</h2>
      <div className="slider-container">
        <div id="other-cards" className="cards-wrapper">
          {renderCards()}
        </div>
